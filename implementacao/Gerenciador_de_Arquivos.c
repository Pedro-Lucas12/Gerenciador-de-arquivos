...
No* comando_cd(No *corrente, char *destino) {
    ...
    while (temp != NULL) {
        ...
        if (strcmp(temp->nome, destino) == 0 && temp->eh_pasta == 1) {
            return temp;
        }
        temp = temp->dir;
    }
...
void liberar_arvore(No* no) {
    if (no == NULL) return;
    liberar_arvore(no->esq); // Libera os filhos (conteúdo da pasta)
    liberar_arvore(no->dir); // Libera os irmãos
    free(no);
}
...
void comando_rm(No *corrente, char *alvo) {
    ...
    while (atual != NULL) {
        if (strcmp(atual->nome, alvo) == 0) {
            ...
            atual->dir = NULL; // Isola o nó (e sua subárvore) do resto da árvore
